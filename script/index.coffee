...
api.wrap = (user, main=true) ->
  ...
  user.fns =
    ...
    cron: (options={}) ->
      ...
      _.each refMerge(user.todos,user.dailys), (task) ->
        return unless task
      ...
      _.each user.todos, (task) -> # Archive todos
    ...
    preenUserHistory: (minHistLen = 7) ->
      _.each refMerge(user.habits,user.dailys), (task) ->
  ...
  Object.defineProperty user, 'tasks',
    get: -> refMerge user.habits, user.dailys, user.todos, user.rewards